...
[Redis Cloud](https://redis.com/cloud/overview/){:target="_blank"} or [AWS
ElastiCache](https://aws.amazon.com/elasticache/redis/){:target="_blank}.
...
[redis/redis-stack](https://redis.io/docs/install/install-stack/docker/){:target="_blank}
so also includes
[RedisInsight](https://redis.io/docs/connect/insight/){:target="_blank} running
on port `8001` that you can use to inspect the Redis server.
...
[Lifespan event
handler](https://fastapi.tiangolo.com/advanced/events/){:target="_blank} as shown
below. Replace the `REDIS_SERVER_URL` with the address and port of your own
Redis server.
...
- The example shown here includes the `sqlalchemy.orm.Session` type, if your
  ...
  docs](https://fastapi.tiangolo.com/tutorial/sql-databases/){:target="_blank}),
  you should include `Session` in `ignore_arg_types` in order for cache keys to
  be created correctly ([More info](#cache-keys)).
...
the FastAPI
docs](https://fastapi.tiangolo.com/tutorial/sql-databases/#create-a-dependency){:target="_blank}:
...
class MLBGameDate:
    def __init__(
        self,
        game_date: str = Query(
            ..., description="Date as a string in YYYYMMDD format"
        ),
