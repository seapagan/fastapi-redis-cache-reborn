...
def test_cache_json_encoder() -> None:
    ...
    assert json_dict["start_time"] == datetime.datetime(2021, 4, 20, 7, 17, 17)
    assert json_dict["finish_by"] == datetime.datetime(2021, 4, 21)
...
def test_if_none_match():
    ...
    assert (
        "x-fastapi-cache" in response.headers
        and response.headers["x-fastapi-cache"] == "Miss"
    )
    assert "cache-control" in response.headers
    assert "expires" in response.headers
    assert "etag" in response.headers
    ...
    assert (
        "x-fastapi-cache" in response.headers
        and response.headers["x-fastapi-cache"] == "Hit"
    )
    assert "cache-control" in response.headers
    assert "expires" in response.headers
    assert "etag" in response.headers
